...
//BOTON EDITAR
async function editarRegistro(id_cliente) {
    try {
        const response = await fetch(`/obtenerRegistro/${id_cliente}`);
        const datos = await response.json();

        if (datos.success) {
            // Guardar ID y datos para el proceso de edición
            localStorage.setItem('editandoId', id_cliente);
            localStorage.setItem('datosEdicion', JSON.stringify(datos.data));
...
//para cargar los datos de edicion
document.addEventListener('DOMContentLoaded', () => {
    const editandoId = localStorage.getItem('editandoId');
    const datosEdicion = JSON.parse(localStorage.getItem('datosEdicion') || '{}');
    
    console.log('Datos recibidos:', datosEdicion);

    if (editandoId && window.location.pathname.includes('principalSistema.html')) {
        // Formatear fecha
        ...
        if (estadoSelect) {
            ...
            setTimeout(() => {
                estadoSelect.value = datosEdicion.id_estado;
                estadoSelect.dispatchEvent(new Event('change'));
            }, 500);
        }

        // Cargar ubicación y propiedad
        document.getElementById('ubicacion').value = datosEdicion.ubicacion || '';
        document.getElementById('propiedad').value = datosEdicion.tipo_propiedad || '';

        // Cargar consumos explícitamente
        if (datosEdicion.consumos && datosEdicion.consumos.length > 0) {
            // Primer bimestre
            document.getElementById('iniPrimero').value = formatearFecha(datosEdicion.consumos[0].fecha_inicio);
            document.getElementById('finPrimero').value = formatearFecha(datosEdicion.consumos[0].fecha_termino);
            document.getElementById('cprimero').value = datosEdicion.consumos[0].consumo;

            // Resto de bimestres
            const bimestres = ['Segundo', 'Tercero', 'Cuarto', 'Quinto', 'Sexto'];
            bimestres.forEach((bimestre, index) => {
                if (datosEdicion.consumos[index + 1]) {
                    document.getElementById(`ini${bimestre}`).value = formatearFecha(datosEdicion.consumos[index + 1].fecha_inicio);
                    document.getElementById(`fin${bimestre}`).value = formatearFecha(datosEdicion.consumos[index + 1].fecha_termino);
                    document.getElementById(`c${bimestre}`).value = datosEdicion.consumos[index + 1].consumo;
                }
            });
        }
    }
...
//         // Modificar botón continuar
//         const btnContinuar = document.querySelector('.continuar');
//         if (btnContinuar) {
//             btnContinuar.textContent = 'Actualizar';
//             btnContinuar.onclick = () => {
//                 if (validarDatosConsumo() && validarDatosUbicacion()) {
//                     actualizarRegistro(editandoId);
//                 }
//             };
//         }
//     }
...
async function actualizarRegistro(id_cliente) {
    const datosActualizados = {
        ...
        consumos: [
            {
                fechaInicio: document.getElementById('iniPrimero').value,
                fechaFin: document.getElementById('finPrimero').value,
                consumo: document.getElementById('cPrimero').value
            },
            {
                fechaInicio: document.getElementById('iniSegundo').value,
                fechaFin: document.getElementById('finSegundo').value,
                consumo: document.getElementById('cSegundo').value
            },
            {
                fechaInicio: document.getElementById('iniTercero').value,
                fechaFin: document.getElementById('finTercero').value,
                consumo: document.getElementById('cTercero').value
            },
            {
                fechaInicio: document.getElementById('iniCuarto').value,
                fechaFin: document.getElementById('finCuarto').value,
                consumo: document.getElementById('cCuarto').value
            },
            {
                fechaInicio: document.getElementById('iniQuinto').value,
                fechaFin: document.getElementById('finQuinto').value,
                consumo: document.getElementById('cQuinto').value
            },
            {
                fechaInicio: document.getElementById('iniSexto').value,
                fechaFin: document.getElementById('finSexto').value,
                consumo: document.getElementById('cSexto').value
            }
        ]
    };

    try {
        const response = await fetch(`/actualizarRegistro/${id_cliente}`, {
            method: 'PUT',
            headers: {
                'Content-Type': 'application/json'
            },
